...
private[akkeeper] object YarnUtils {
  ...
  def defaultStagingDirectory(conf: Configuration): String = {
    new Path(FileSystem.get(conf).getHomeDirectory, ".akkeeper").toString
  }

  def appStagingDirectory(conf: Configuration, base: Option[String], appId: String): String = {
    new Path(base.getOrElse(defaultStagingDirectory(conf)), appId).toString
  }
}
