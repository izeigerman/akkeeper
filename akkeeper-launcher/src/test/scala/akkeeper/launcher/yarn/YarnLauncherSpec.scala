...
class YarnLauncherSpec extends FlatSpec with Matchers with MockFactory with BeforeAndAfterAll {
  ...
  "A YARN Launcher" should "launch application master successfully" in {
    ...
    val result = Await.result(launcher.launch(config, launchArgs), Duration.Inf)
    result.appId shouldBe appId.toString
    result.masterHost shouldBe "localhost"
  ...
  it should "throw exception if the application state is FAILED" in {
    ...
    intercept[YarnLauncherException] {
      Await.result(launcher.launch(config, launchArgs), Duration.Inf)
    }
  }
}
